...
class DVP(commands.Cog):
    ...
    async def get_channel_games(self):
        ...
        headers = {
            ...
            'Authorization': f'Bearer {self.bot.twitch_api.oauth_token}'
        }
    ...
    async def update_google_sheet(self):
        ...
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT name, time_played, last_played FROM games ORDER BY last_played DESC") as cursor:
            ...
            requests.append({
                "updateCells": {
                    "rows": [{
                        "values": [{
                            "userEnteredValue": {
                                "stringValue": f"Last Updated: {current_date}"
                            },
                            "userEnteredFormat": {
                                "horizontalAlignment": "CENTER",
                                "textFormat": {
    ...
    async def did_vulpes_play_it(self, ctx: commands.Context, *, game_name: str):
        ...
        try:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                async with db.execute("SELECT time_played, last_played FROM games WHERE name = ?", (game_name_to_search,)) as cursor:
            ...
            if result:
                ...
                await ctx.send(f"@{ctx.author.name}, Vulpes played {game_name_to_search} for {int(hours)}h {int(minutes)}m. Last played on {last_played_formatted}.")
            else:
